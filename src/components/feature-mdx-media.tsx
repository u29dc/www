 ...
 * ## USAGE
 * ```mdx
 * <FeatureMdxMedia src={["Square.webp"]} />                        <!-- Single item -->
 * <FeatureMdxMedia src={["Square.webp", "Landscape.webp"]} />      <!-- Multiple items -->
 * ```
 ...
 *
 * @module components/feature-mdx-media
 */
...
export function FeatureMdxMedia({ src, alt }: FeatureMdxMediaProps) {
	...
	return (
		<div className="grid grid-cols-10 col-span-full">
			<MediaLayoutContext.Provider value={contextValue}>
				...
				>
					{src.map((source) => (
						<FeatureMdxMediaItem key={source} src={source} alt={alt || ''} />
					))}
				</div>
