...
const UNIFORM_NAMES = ['u_resolution', 'u_time', 'u_intensity', 'u_exposure', 'u_grainRepeat', 'u_chromaticVariance', 'u_scrambleOffset', 'u_blueNoise', 'u_lowFreqSeed'] as const;
...
const loadBlueNoisePixels = async (): Promise<Uint8Array> => {
	...
	blueNoisePixelsPromise = (async () => {
		...
		if (bytes.length !== expectedSize) {
			throw new Error(`Unexpected blue-noise payload size: ${bytes.length}.`);
		}
		return bytes;
	})();
	return blueNoisePixelsPromise;
...
function getDprCap(): number {
	...
	const isMobile = typeof window !== 'undefined' && (window.matchMedia?.('(max-width: 768px)').matches ?? window.innerWidth <= 768);
...
function measureCanvas(canvas: HTMLCanvasElement, dprCap: number): CanvasDimensions {
	...
	const isFixedViewportOverlay = computed.position === 'fixed' && computed.left === '0px' && computed.top === '0px' && computed.right === '0px' && computed.bottom === '0px';
...
function createGrainRenderer(canvas: HTMLCanvasElement, initialState: GrainState, blueNoisePixels: Uint8Array): GrainRenderer {
	...
	gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, BLUE_NOISE_SIZE, BLUE_NOISE_SIZE, 0, gl.RGBA, gl.UNSIGNED_BYTE, blueNoisePixels);
	...
	const updateResolutionUniforms = () => {
		setUniform2f(uniforms.u_resolution, dimensions.pixelWidth, dimensions.pixelHeight, gl);
	};
	...
	const computeScrambleOffset = (index: number, isFrozen: boolean, repeats: { x: number; y: number }) => {
		if (isFrozen) {
			return [0, 0] as const;
		}
	...
	const drawFrame = (timestamp: number) => {
		...
		const time = state.isMotionDisabled ? state.timeOffset : timestamp * state.animationSpeed + state.timeOffset;
		const repeats = computeRepeats(dimensions.width, dimensions.height, state.grainScale);
		const scrambleIndex = frameCounter % SCRAMBLE_OFFSETS.length;
		const [scrambleX, scrambleY] = computeScrambleOffset(scrambleIndex, state.isMotionDisabled, repeats);
		frameCounter += 1;
		...
		setUniform1f(uniforms.u_chromaticVariance, state.chromaticVariance, gl);
...
const adjustedIntensity = $derived(resolvedTheme === 'light' ? intensity * 1.1 : intensity * 0.8);
const isActive = $derived(deviceTier !== 'low' && isPageVisible && !prefersReducedMotion && isInView);
...
onMount(() => {
	...
	const initRenderer = async () => {
		try {
			...
			renderer = createGrainRenderer(canvasRef, grainState, blueNoisePixels);
