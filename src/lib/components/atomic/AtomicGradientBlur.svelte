...
const calculateBlur = (layerIndex: number, totalLayers: number, intensity: number, selected: BlurCurve, useExponential: boolean): number => {
...
const generateMaskGradient = (layerIndex: number, totalLayers: number): string => {
...
const layerStyles = $derived(
	Array.from({ length: layers }, (_, index) => {
		const blurValue = calculateBlur(index, layers, strength, curve, exponential);
...
const containerStyle = $derived(
	[
		...
		`${position}: 0`,
		isVertical ? `height: ${size}; width: 100%; left: 0; right: 0` : `width: ${size}; height: 100%; top: 0; bottom: 0`,
	].join('; '),
);
