<script lang="ts">
	...
	interface AtomicBrandLogoRenderer {
		resize(
			dimensions?: Partial<Pick<CanvasDimensions, "width" | "height">>,
		): void;
	...
	function createAtomicBrandLogoRenderer(
		...
		const updateResolutionUniforms = (nextDimensions: CanvasDimensions) => {
			...
			setUniform2f(
				...
				nextDimensions.pixelWidth,
				nextDimensions.pixelHeight,
				gl,
			);
		...
		const applyStaticUniforms = (current: AtomicBrandLogoState) => {
			...
			setUniform1f(
				uniforms.u_defaultBlurIntensity,
				current.defaultBlurIntensity,
				gl,
			);
			setUniform1f(uniforms.u_mouseBlurSize, current.mouseBlurSize, gl);
			setUniform1f(
				uniforms.u_mouseBlurIntensity,
				current.mouseBlurIntensity,
				gl,
			);
		...
		const applyThemeUniforms = (variant: ThemeVariant) => {
			...
			setUniform1f(
				uniforms.u_heightSpreadMultiplier,
				heightMultiplier,
				gl,
			);
		...
		const resize = (
			nextDimensions?: Partial<
				Pick<CanvasDimensions, "width" | "height">
			>,
		) => {
