...
type CachedLinePlan = LinePlanSignature & {
	targetWidth: number;
	targetHeight: number;
...
const LINE_TARGET_SELECTOR = '[data-line-reveal]';
...
const targetOrder = (target: HTMLElement): number => {
	...
	return Array.from(document.querySelectorAll(LINE_TARGET_SELECTOR)).indexOf(target);
};
...
const readLineContentSignature = (target: HTMLElement): string => hash([target.textContent ?? '', atomicSignature(target)].join('::'));

const readLineTypographySignature = (target: HTMLElement): string => {
...
const createLinePlanSignature = (target: HTMLElement, targetRect: Rect, profile: LineRevealProfile, typographySignature = readLineTypographySignature(target)): LinePlanSignature => {
...
const readCachedLinePlan = (signature: LinePlanSignature, targetRect: Rect): CachedLinePlan | undefined => {
	const plan = planCache.get(signature.key);
	...
	if (plan.contentSignature !== signature.contentSignature || plan.typographySignature !== signature.typographySignature || plan.profile !== signature.profile) {
		planCache.delete(signature.key);
		return undefined;
	}
...
const enforceCacheLimit = (): void => {
	if (planCache.size <= MAX_CACHE_ENTRIES) return;

	const entries = Array.from(planCache.values()).toSorted((a, b) => a.lastUsed - b.lastUsed);
	while (planCache.size > MAX_CACHE_ENTRIES) {
		const entry = entries.shift();
		if (!entry) return;
		planCache.delete(entry.key);
	}
};

const writeCachedLinePlan = (signature: LinePlanSignature, targetRect: Rect, lineRects: Rect[]): void => {
	if (!canWriteLinePlan()) return;

	const now = performance.now();
	planCache.set(signature.key, {
		...signature,
